...
2. 기본 토핑: 도우, 소스, 치즈 (항상 포함)
3. 추가 토핑: 페퍼로니, 소세지, 피망, 가지 중 선택 (절대 다른 토핑은 사용하지 마세요, 반드시 주어진 토핑만 사용하세요)
4. 반드시 "사용된 토핑:" 다음에 쉼표로 구분된 토핑 리스트를 작성하세요
5. 딕셔너리나 JSON 형식을 사용하지 마세요
...
제 친구는 채식주의자예요. 근데 야채랑 과일을 안 좋아해요. 버섯류는 좋아하는데, 피자에는 별로래요.
사용된 토핑: 도우, 소스, 치즈
...
### 주의 사항 ###
도우, 소스, 치즈, 소세지, 페퍼로니, 피망, 가지 외의 다른 토핑을 절대로 사용하지 마세요 (버섯, 파프리카, 올리브 오일 등 ❌)
반드시 형식에 맞게 사용된 토핑을 같이 출력해주세요
"""
...
# 결과 화면
    ...
    with col2:
        st.metric("최종 보유 금액", f"{final_money:,}원", delta=f"{final_money:,}원")
    ...
    st.write("")
    st.write(f"총 {st.session_state.submitted_count}명의 손님을 응대했습니다.")
